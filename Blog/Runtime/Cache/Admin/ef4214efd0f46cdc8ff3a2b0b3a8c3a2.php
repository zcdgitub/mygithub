			...
			<div class="widget-box">
				...
				<div class="widget-content">
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($data["data"])): $i = 0; $__LIST__ = $data["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr >
								<td class="id" ><?php echo ($v["g_id"]); ?></td>
								<td><?php echo ($v["g_name"]); ?></td>
								<td><?php echo (msubstr(htmlspecialchars_decode($v["g_content"]),0,120,'utf-8',true)); ?></td>
								<td><?php echo (date("Y-m-d H:i:s",$v["g_time"])); ?></td>
								<td>
									<?php if(($v['g_rtime']) == "0"): ?><span class=" btn-dark-red btn-xs">未处理</span>
										<?php else: ?>
										<span class=" btn-dark-green btn-xs">已处理</span><?php endif; ?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('edit','id='.$v['g_id']);?>">编辑</a>&nbsp;&nbsp;
									<button class="btn btn-danger btn-sm delgust" data-img-id="<?php echo ($v["g_id"]); ?>">删除</button>
								</td>
