...
<div id="sidebar">
				...
				<ul>
					...
					<li><a href="<?php echo U('admin/Index/index');?>" target="_blank"><i class="fa fa-home"></i> <span>后台首页</span></a></li>
			...
			<div class="widget-box">
				...
				<div class="widget-content">
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($page["data"])): $i = 0; $__LIST__ = $page["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center">
								<td class="id"><?php echo ($vo["s_id"]); ?></td>
								<td><?php echo (msubstr(htmlspecialchars_decode($vo["s_content"]),0,120,'utf-8',true)); ?></td>
								<td><?php echo (date('Y-m-d H:i:s',$vo["s_time"])); ?></td>
								<td>
									<?php if(($vo["s_view"]) == "0"): ?><span class=" btn-dark-red btn-xs">不显示</span>
										<?php else: ?>
										<span class=" btn-dark-green btn-xs">显示</span><?php endif; ?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('edit','id='.$vo['s_id']);?>">编辑</a>&nbsp;&nbsp;
								
								<input type="button" class="btn btn-danger btn-sm delsay" data-img-id="<?php echo ($vo["s_id"]); ?>" value="删除" id="bt" onclick="return confirm('确定要删除吗?')">
								</td>
