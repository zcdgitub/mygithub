					...
					<table class="table table-bordered table-striped table-hover ">
						...
						<tbody>
							<?php if(is_array($page["data"])): $i = 0; $__LIST__ = $page["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center">
								...
								<td><?php echo (msubstr($vo["t_remark"],0,20,'utf-8',true)); ?></td>
								<td><?php echo (date("Y-m-d H:i:s",$vo["t_time"])); ?></td>
								<td><?php echo ($vo["t_parent_id"]); ?></td>
								<td>
								<?php if(($vo["t_view"]) == "0"): ?><span class=" btn-dark-red btn-xs">不显示</span>
								<?php else: ?>
										<span class=" btn-dark-green btn-xs">显示</span><?php endif; ?>
								
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('edit','id='.$vo['t_id']);?>">编辑</a>&nbsp;&nbsp;
									
									<input type="button" class="btn btn-danger btn-sm deltag jsdel" data-img-id="<?php echo ($vo["t_id"]); ?>" id="bt" onclick="return confirm('确定要删除吗?')" value="删除">
								</td>
