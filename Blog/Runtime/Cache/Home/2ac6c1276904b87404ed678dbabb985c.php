...
<!-- http://www.jq22.com/jquery -->
    <div class="row aerousel">
		...
		<h5>
			<?php if(($data["a_original"]) == "1"): ?><span class="original">【原创】</span>
			<?php else: ?>
				<span class="reprint">【转载】</span><?php endif; ?>
			<a href=""><?php echo ($data["a_title"]); ?></a>
		</h5>
           	<div class="a-write">栏目：<?php echo ($data["t_name"]); ?>&nbsp;&nbsp;作者：<?php echo ($data["a_author"]); ?>&nbsp;&nbsp;阅读：（<?php echo ($data["a_hit"]); ?>）<span class="hidden-xs">更新于：<?php echo (getTime($data["a_time"])); ?></span></div>
            <div class="a-content"><?php echo (htmlspecialchars_decode($data["a_content"])); ?></div>
            <div class="article-copy hidden-xs">
                <?php if(($data["a_original"]) == "1"): ?><p>非特殊说明，本文版权归<?php echo ($data["a_author"]); ?>所有，转载请注明出处.</p>
                	<p>本文标题：<?php echo ($data["a_title"]); ?></p>
                <!--     <p>本文网址：<a><?php echo (C("SITE_URL")); ?>.com/article-<?php echo ($info["a_id"]); ?>.html</a>.</p> -->
                	<p>本文标签：<a><?php echo ($data["a_keyword"]); ?></a></p>
                <?php else: ?>
            ...
            <hr />
                ...
                <div class="a-up"><p>上一篇：<?php if(empty($prev["a_id"])): ?>&nbsp;没有了<?php else: ?><a title='<?php echo ($prev["a_title"]); ?>' href="<?php echo U('/article/index','id='.$prev['a_id']);?>">&nbsp;<?php echo (msubstr($prev["a_title"],0,40,'utf-8',ture)); ?></a><?php endif; ?></p></div>
                 <div class="a-down"><p>下一篇：<?php if(empty($next["a_id"])): ?>&nbsp;没有了<?php else: ?><a title='<?php echo ($next["a_title"]); ?>' href="<?php echo U('/article/index','id='.$next['a_id']);?>">&nbsp;<?php echo (msubstr($next["a_title"],0,40,'utf-8',ture)); ?></a><?php endif; ?></p></div>

           
           <div class="bshare-custom"> 
           <span class="">分享到:</span>
                    <div class="bsPromo bsPromo2"></div>
                    <a title="分享到QQ空间" class="bshare-qzone"></a><a title="分享到微信" class="bshare-weixin" href="javascript:void(0);"></a><a title="分享到新浪微博" class="bshare-sinaminiblog"></a><a title="分享到豆瓣" class="bshare-douban" href="javascript:void(0);"></a><a title="分享到人人网" class="bshare-renren"></a><a title="更多平台" class="bshare-more bshare-more-icon more-style-addthis"></a>
                </div>
                <script type="text/javascript" charset="utf-8" src="http://static.bshare.cn/b/buttonLite.js#style=-1&amp;uuid=&amp;pophcol=2&amp;lang=zh"></script>
                <script type="text/javascript" charset="utf-8" src="http://static.bshare.cn/b/bshareC0.js"></script>
            <hr />  

        <?php if(is_array($article_c)): $i = 0; $__LIST__ = $article_c;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="media connect" style="padding-left:<?php echo ($vo['lev']+1); ?>cm">
                <div class="media-left">
    	             
    	              <!--   <img src="<?php echo ($vo["ac_img"]); ?>" class="media-object img-circle img-50" alt="<?php echo ($vo["ac_name"]); ?>"/> -->
    	               <img src="<?php echo ($vo["ac_img"]); ?>" class="media-object img-circle img-50" alt="<?php echo ($vo["ac_name"]); ?>"/>
                   
                </div>
                <div class="media-body">
                   <!--  <div class="fool">#<?php echo ($count); ?>楼</div> -->
                    <div class="media-heading">
                            <a>
                                <span class="glyphicon glyphicon-user" ><?php echo ($vo["ac_name"]); ?> </span> 
                            </a>                     
                             <div class="connect-box"><span ><?php echo (reFace($vo["ac_content"])); ?></span></div>
                    </div>
                      
                    <div class="date-dz">
                        <a ><span class=" glyphicon glyphicon-time"><?php echo (date("Y-m-d H:i:s",$vo["ac_time"])); ?></span></a>

                        <div class="date-dz-right pull-right comment-pl-block">
                            <a href="javascript:;" class="date-dz-pl hf-con-block hf" id="<?php echo ($vo["ac_id"]); ?>" >回复</a>
    ...
    <textarea class="form-control" id="content" name="g_content" rows="3" placeholder="请输入评论内容" rows="7"></textarea>
                            ...
                            <div class="form-group home-from-box">
                                <p> 
                                    ...
                                    <button class="btn btn-default btn-add" style="float: right;">提交</button> 
                                </p>
                            </div>
...
$('.btn').click(function (){
   var name=$(this).parent().parent().siblings().find('#name').val();
 /*  var name=$('#name').val();*/

   var content=$(this).parent().parent().siblings().find('#content').val();
   var pid=$(this).parent().parent().siblings().find('#pid').val();
   var aid=$(this).parent().parent().siblings().find('#aid').val();
 ...
 $(".hf").click(function (){
        ...
        if($(this).parent().parent().next().children().hasClass('well')){
                $(this).parent().parent().next().children('.well').remove();
        }else{
           /* var id=$(this).attr('id');//子评论id值
                console.log(id);*/

            var name2=$(this).parent().parent().siblings('.media-heading').find('a').find('span').html(); 

            var huifu='@'+name2+':';
             /*   console.log(huifu);*/
              var fhHtml = '<div class="well">'+
              '<div class="input-group home-from-box">'+
                    '<span class="input-group-addon" id="basic-addon1">昵称</span>'+
                    '<input type="text" class="form-control name" placeholder="昵称" aria-describedby="basic-addon1" name="g_name"  id="name">'+
                    '<input type="hidden" name="p_id" value="<?php echo ($vo["ac_id"]); ?>" id="pid"/>'+
                    '<input type="hidden"  name="ac_pid"    value="<?php echo ($data["a_id"]); ?>"  id="aid" />'+
                    '<span class="glyphicon glyphicon-remove form-control-feedback hidden" aria-hidden="true"></span>'+
                '</div>'+
                '<div class="form-group home-from-box text">'+

                    '<textarea class="form-control" id="content" name="g_content" rows="3"  rows="7" value="<?php echo ($vo["ac_name"]); ?>"></textarea>'+
                '</div>'+
                    '<div class="form-group home-from-box">'+
                        '<p>'+
                        '<span class="emotion1">表情</span>'+
                        '<button class="btn btn-default btn-add" style="float: right;">提交</button>'+
                        '</p>'+
                    '</div>'+
        '</div>';
          $(this).parent().parent().next('#dongtai').append(fhHtml);
              $(this).parent().parent().next('#dongtai').find('.well').find('.text').find('.form-control').val(huifu);
              $('.emotion1').qqFace({
                    id: 'facebox',
                   assign:'content',
                   path:'/Public/Arclist/'
        ...
        $('.btn').click(function (){
            ...
            var name1=$(this).parent().parent().siblings().find('#name').val();//评论人
            //var name2=$(this).parent().parent().parent().parent().siblings('.media-heading').find('a').find('span').html(); //
           /* console.log(name1);*/
            var content=name1+$(this).parent().parent().siblings().find('#content').val();
           /* console.log(content);*/
            var pid=$(this).parent().parent().parent().parent().siblings('.date-dz').find('.date-dz-right').find('a').attr('id');
    
           
           /* $(this).parent().parent().siblings().find('#pid').val(pid);*/
            var aid=$(this).parent().parent().siblings().find('#aid').val();
